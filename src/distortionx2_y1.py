...
""" Ce script à pour but de corriger la distortion issues des détections à partir de la matrice intrinsèques, de la distortion obtenue lors du calibrage des données.
    Ce script est efficace pour les angles de vues dont on se charge de récupérer les coordonnées X2, Y1"""
...
def import_detections(detections_cam): 
    ...
    detections = detections[['frame','id', 'x2', 'y1']]
    detections2 = detections.rename(columns={'x2': 'x1', 'y1': 'y1'})
    return detections2
...
def undistort_points(row, K, D): 
    ...
    # Récupérer les colonnes x1 et y1
